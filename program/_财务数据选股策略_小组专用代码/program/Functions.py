...
def save_input_data(df):
    ...
    df = df[['交易日期', '股票名称', '总市值', '换手率','换手率_5avg','换手率_20avg','中户买入占比_5avg','中户买入占比_20avg','散户卖出占比','散户卖出占比_5avg','散户卖出占比_20avg','rsi', '5日均线', 'bias', '5日累计涨跌幅', '申万一级行业名称',
    ...
    df["申万一级行业代码"] = pd.factorize(df["申万一级行业名称"])[0].astype(int)
    df['涨跌幅分类'] = df['下周期总涨跌幅'].apply(lambda x: test1(x))
    # df.dropna(subset=['交易日期', '股票名称', '总市值', '换手率','换手率_5avg','换手率_20avg','中户买入占比_5avg','中户买入占比_20avg','散户卖出占比','散户卖出占比_5avg','散户卖出占比_20avg','rsi', '5日均线', 'bias', '5日累计涨跌幅', '申万一级行业名称',
    #           '归母ROE(ttm)', '归母EP(ttm)', '现金流负债比','总市值_分位数','归母ROE比120_分位数', 'BP_排名','存货周转率',
    #          '归母EP(ttm)_二级行业分位数','经营活动现金流入小计_分位数','10VWAP','成交额_排名','10日累计涨跌幅','120日累计涨跌幅','250日累计涨跌幅','下周期总涨跌幅'], how='any', inplace=True,
    #           axis=0)
    df.fillna(value= -1, inplace=True)
    df.drop(['交易日期', '股票名称', '申万一级行业名称', '下周期总涨跌幅'], axis=1, inplace=True)
    df.to_csv('/Users/lishechuan/python/coincock/data/模型数据/model.csv', index=False)
    print("已经保存完训练数据！")