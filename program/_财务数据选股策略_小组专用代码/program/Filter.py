...
def filter_and_rank_W(df):
    ...
    condition &= (df['均线_5'] < df['均线_31'] * 1.2)
...
def filter_and_rank_yuanshi(df):
    """
    通过财务因子设置过滤条件
    :param df: 原始数据
    :return: 返回 通过财务因子过滤并叠加量价因子的df
    ...
    factors_rank_dict = [
        ...
        ('成交额std_10_排名', True, 0.2),
        ('量价相关性_10_排名', True, 1),
        ('量价相关性_30_排名', True, 0.1),
        ('成交额std_20_排名', True, 0.6),
        ('Illiquidity_排名', True, 2),
        ('L97_排名', True, 0.2),
        ('cro_3w_排名', True, 0.2),
        ('cro_2w_排名', True, 0.5),
        ('cro_m_排名', False, 0.5),
