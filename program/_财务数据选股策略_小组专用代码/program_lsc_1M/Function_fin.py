...
def proceed_fin_data(finance_df, raw_fin_cols, flow_fin_cols, cross_fin_cols, derived_fin_cols):
    ...
    finance_df_ = cal_fin_data(df=finance_df, flow_fin_list=flow_fin_cols, cross_fin_list=cross_fin_cols, discard=False)

    # ======================添加的代码======================
    finance_df = finance_df_.copy()
    # 进行去除废弃财报
    finance_df['废弃报告'] = mark_old_report(finance_df['report_date'])
    # 删除废弃的研报
    finance_df = finance_df[finance_df['废弃报告'] != 1]
    # 删除不必要的行
    del finance_df['废弃报告']
    # ======================添加的代码======================
    ...
    return finance_df[['publish_date', 'report_date'] + cols], finance_df_
