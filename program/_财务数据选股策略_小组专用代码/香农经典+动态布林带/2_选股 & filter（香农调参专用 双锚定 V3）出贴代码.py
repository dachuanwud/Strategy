...
def elements(df,x,y): # 锚定指标：几何年化收益率
    ...
    df['归母EP(ttm)_二级行业分位数'] = df.groupby(['交易日期', '申万二级行业名称'])['归母EP(ttm)'].rank(ascending=True,
                                                                                                        pct=True)
    ...
    df['中户买入占比_20avg_分位数'] = df.groupby(['交易日期'])['中户买入占比_20avg'].rank(ascending=True, pct=True)
    condition &= (df['中户买入占比_20avg'] >= x)
    condition &= (df['散户卖出占比_20avg'] >= y)
