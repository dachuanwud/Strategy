    ...
    for root, dirs, files in os.walk(path):
        if files:  # 当files不为空的时候
            for f in files:
                ...
                if f.startswith('bj8'):
                    continue
                if f.startswith('sh68'):
                    continue
                # if f.startswith('sz30'):
                #     continue
                if f.endswith('.csv'):
...
def import_index_data(path, back_trader_start=None, back_trader_end=None):
    ...
    df_index = df_index[['candle_end_time', '指数涨跌幅', 'high', 'low', 'open', 'close']]
...
def transfer_to_period_data(df, period_type='m', extra_agg_dict={}):
    ...
    # 计算周期择时后涨跌幅
    period_df['每天涨跌幅_择时'] = df['涨跌幅_择时'].resample(period_type).apply(lambda x: list(x))
    period_df['涨跌幅_择时'] = df['涨跌幅_择时'].resample(period_type).apply(lambda x: (x + 1).prod() - 1)
...
# 计算策略评价指标
def strategy_evaluate(equity_df, select_stock, type_list=['']):
    """
    :param col_list:
    ...
    year_return = equity[['涨跌幅', '涨跌幅_择时', '指数涨跌幅']].resample(rule='A').apply(lambda x: (1 + x).prod() - 1)
    month_return = equity[['涨跌幅', '涨跌幅_择时', '指数涨跌幅']].resample(rule='M').apply(lambda x: (1 + x).prod() - 1)

    year_return['超额收益'] = year_return['涨跌幅'] - year_return['指数涨跌幅']
    month_return['超额收益'] = month_return['涨跌幅'] - month_return['指数涨跌幅']

    year_return['超额收益_择时'] = year_return['涨跌幅_择时'] - year_return['指数涨跌幅']
    month_return['超额收益_择时'] = month_return['涨跌幅_择时'] - month_return['指数涨跌幅']
    ...
    for type in type_list:
        ...
        # 计算最大回撤开始时间
        start_date = equity[equity['交易日期'] <= end_date].sort_values(by=col, ascending=False).iloc[0]['交易日期']
        # 将无关的变量删除
        ...
        # ===统计每个周期
        results.loc[col, '盈利周期数'] = len(select_stock.loc[select_stock['选股下周期涨跌幅' + type] >= 0])  # 盈利笔数
        results.loc[col, '亏损周期数'] = len(select_stock.loc[select_stock['选股下周期涨跌幅' + type] < 0])  # 亏损笔数
        results.loc[col, '盈利胜率'] = format(results.loc[col, '盈利周期数'] / len(select_stock), '.2%')  # 胜率

        results.loc[col, '每周期平均收益'] = format(select_stock['选股下周期涨跌幅' + type].mean(), '.2%')  # 每笔交易平均盈亏
        results.loc[col, '盈亏收益比'] = round(select_stock.loc[select_stock['选股下周期涨跌幅'] > 0]['选股下周期涨跌幅' + type].mean() / \
                                          select_stock.loc[select_stock['选股下周期涨跌幅' + type] <= 0][
                                              '选股下周期涨跌幅' + type].mean() * (-1), 2)  # 盈亏比

        results.loc[col, '单周期最大盈利'] = format(select_stock['选股下周期涨跌幅' + type].max(), '.2%')  # 单笔最大盈利
        results.loc[col, '单周期大亏损'] = format(select_stock['选股下周期涨跌幅' + type].min(), '.2%')  # 单笔最大亏损

        # ===连续盈利亏损
        results.loc[col, '最大连续盈利周期数'] = max(
            [len(list(v)) for k, v in
             itertools.groupby(np.where(select_stock['选股下周期涨跌幅' + type] >= 0, 1, np.nan))])  # 最大连续盈利次数
        results.loc[col, '最大连续亏损周期数'] = max(
            [len(list(v)) for k, v in
             itertools.groupby(np.where(select_stock['选股下周期涨跌幅' + type] < 0, 1, np.nan))])  # 最大连续亏损次数

        # ===统计每个月
        results.loc[col, '跑赢基准月数'] = len(month_return.loc[month_return['超额收益' + type] > 0])
        results.loc[col, '跑输基准月数'] = len(month_return.loc[month_return['超额收益' + type] <= 0])
        results.loc[col, '跑赢基准月胜率'] = format(results.loc[col, '跑赢基准月数'] / len(month_return), '.2%')  # 胜率

        results.loc[col, '单月最大盈利'] = format(month_return['涨跌幅' + type].max(), '.2%')  # 单笔最大盈利
        results.loc[col, '单月最大亏损'] = format(month_return['涨跌幅' + type].min(), '.2%')  # 单笔最大亏损

        results.loc[col, '单月最大超额收益'] = format(month_return['超额收益' + type].max(), '.2%')  # 单笔最大盈利
        results.loc[col, '单月最小超额收益'] = format(month_return['超额收益' + type].min(), '.2%')  # 单笔最大亏损

        results.loc[col, '月均超额收益'] = format(month_return['超额收益' + type].mean(), '.2%')  # 单笔最大亏损

    return results.T, year_return, month_return
