...
def calculate_by_stock(code):
    ...
    # lsc 新加的
    df['周均线_20'] = df['收盘价_复权'].rolling(window=20).mean()
    df['周_macd'] = df['收盘价_复权'].ewm(alpha=2 / 13, adjust=False).mean() - df['收盘价_复权'].ewm(alpha=2 / 27,
                 adjust=False).mean()
...
if __name__ == '__main__':
    ...
    # 将数据存储到pickle文件
    all_stock_data.to_pickle(root_path + '/data/output/选股策略/lsc_all_stock_data_' + period_type + '.pkl')
