...
def boll_signal(df):
    ...
    df['boll_lower_斜率'] = df['boll_lower'] / df['boll_lower'].shift(1)
    df['boll_lower_斜率'] = df['boll_lower_斜率'].apply(lambda x: math.atan((x - 1) * 100) * 180 / 3.1415926)

    # 做多信号
    condition1 = df['收盘价_复权'] <= df['boll_lower']
    condition2 = df['收盘价_复权'] > df['boll_lower'].shift(1)

    df.loc[condition1 & condition2, 'signal'] = 1
    # 做空信号
    condition1 = df['收盘价_复权'] >= df['boll_upper']
    condition2 = df['收盘价_复权'] < df['boll_upper'].shift(1)
    df.loc[condition1 & condition2, 'signal'] = 0

    df = df[df['signal'] == 1]
    print(df[['交易日期', '股票名称','收盘价_复权', 'boll_lower', 'boll_lower_斜率', 'signal']].tail(100))
    exit()
    return df
def BIAS_signal(df, para):
    # 三个参数
    n1 = para[0]
    n2 = para[1]
    n3 = para[2]
    ...
    # ==删除无关变量
    df.drop(['ma_long', 'ma_short', 'signal_long', 'signal_short'], axis=1, inplace=True)

    return df

def simple_moving_average_para_list(ma_short=range(10, 200, 10), ma_mid = range(10, 200, 10), ma_long = range(10, 300, 10)):
    """
    产生简单移动平均线策略的参数范围
    :param ma_short:
    :param ma_long:
...
def price_bottom(df, para):
    ma_short = para[0]
    ma_mid = para[1]
    ma_long = para[2]

    df['ma_short'] = df['收盘价_复权'].rolling(ma_short, min_periods=1).mean()
    df['ma_mid'] = df['收盘价_复权'].rolling(ma_long, min_periods=1).mean()
    df['ma_long'] = df['收盘价_复权'].rolling(ma_long, min_periods=1).mean()

    # 1.寻找第一个金叉，5 穿 10
    condition1 = df['ma_short'] > df['ma_mid']  # 短期均线 > 长期均线
    condition2 = df['ma_short'].shift(1) <= df['ma_mid'].shift(1)  # 上一周期的短期均线 <= 长期均线
    df.loc[condition1 & condition2, 'signal_a'] = 1  # 将产生做多信号的那根K线的signal设置为1，1代表做多

    # 2.寻找第一个金叉，5 穿 20
    condition1 = df['ma_short'] > df['ma_long']  # 短期均线 > 长期均线
    condition2 = df['ma_short'].shift(1) <= df['ma_long'].shift(1)  # 上一周期的短期均线 <= 长期均线
    df.loc[condition1 & condition2, 'signal_b'] = 1  # 将产生做多信号的那根K线的signal设置为1，1代表做多

    # 3.寻找第一个金叉，10 穿 20
    condition1 = df['ma_mid'] > df['ma_long']  # 短期均线 > 长期均线
    condition2 = df['ma_mid'].shift(1) <= df['ma_long'].shift(1)  # 上一周期的短期均线 <= 长期均线
    df.loc[condition1 & condition2, 'signal_c'] = 1  # 将产生做多信号的那根K线的signal设置为1，1代表做多
